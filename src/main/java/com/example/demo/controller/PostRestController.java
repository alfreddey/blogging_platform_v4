...
import com.example.demo.service.PostPermissionService;
...
public class PostRestController {
    ...
    @PostMapping
    public ApiResponse<PostResponse> create(@Valid @RequestBody PostRequest request, Authentication auth) {
    ...
    public ApiResponse<Void> delete(@PathVariable String postId, Authentication auth) {
        String email = auth.getName();

        if (!postPermissionService.canEditPost(postId, email)) {
            throw new RuntimeException("User with email: %s does not have permission to delete post with id: %s".formatted(email, postId));
        }
    ...
    public ApiResponse<PostResponse> updatePostContent(@Valid @PathVariable String postId, @RequestBody PostRequest request, Authentication auth) {
        String email = auth.getName();

        if (!postPermissionService.canEditPost(postId, email)) {
            throw new RuntimeException("User with email: %s does not have permission to edit post with id: %s".formatted(email, postId));
        }
